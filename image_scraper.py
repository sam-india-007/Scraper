import requests, os, time, unicodedata
import re, shutil
...
for url in urls:
    ...
    for book in books:
        ...
        image = image[12:]
        full_url = "http://books.toscrape.com/"+image
        
        #if r.status_code == 200:
        #    with open("images/book"+str(i)+".jpg", 'wb') as f: 
        #        r.raw.decode_content = True
        #        shutil.copyfileobj(r.raw, f)
        for cat in cats:
            if re.search(cat, url) is not None:
                mypath = "book_images/"+cat
